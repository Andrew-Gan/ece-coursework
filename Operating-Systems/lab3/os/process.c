...
//	done in assembly language elsewhere.a
...
// to happen).
static Queue	runQueue[NUM_QUEUES];
...
//The decay process when a process is woken up 
void ProcessDecayEstcpuSleep(PCB *pcb) {
  int i;
  int num_windows_asleep;
  float total;
 
  num_windows_asleep = pcb->sleeptime / (TIME_PER_CPU_WINDOW * CPU_WINDOWS_BETWEEN_DECAYS);
  total = 1.0f;
  for (i = 0; i < num_windows_asleep; i++){
    total *= 2.0f/3.0f;
  }
  pcb->estcpu = pcb->estcpu * total;
}

//Find the Highest Priority from PCB by checking each queue
PCB* ProcessFindHighestPriorityPCB() {
  PCB* highest_priority_pcb; 
  int i;
  
  for(i = 0; i < NUM_QUEUES; i++) {
    if(!AQueueEmpty(&runQueue[i])) {
      highest_priority_pcb = (PCB *)AQueueObject(runQueue[i].first);
      break;
    }
  }
...
void ProcessDecayAllEstcpus(){
  ...
  int i;
  Link *l;
  // update all priorities and estcpu for all processes in all runQueues
  for(i = 0; i < NUM_QUEUES; i++) {
    l = AQueueFirst(&runQueue[i]);
    while (l != NULL ) {
      pcb = (PCB *)AQueueObject(l);
      if(pcb != idlePCB){
          pcb->estcpu = (pcb->estcpu * (2.0f / 3.0f)) + pcb->pnice;
...
// Insert all processes to proper queue
void ProcessFixRunQueues() {
  ...
  for(q = 0; q < NUM_QUEUES; q++) {
    l = AQueueFirst(&runQueue[q]);
    nitems = runQueue[q].nitems;
    for (i = 0 ; l != NULL && i < nitems; i ++ ){
      pcb = (PCB *)AQueueObject(l);
      l = AQueueNext(l);
...
int ProcessCountAutowake(){
  ...
  for(i = 0; i < PROCESS_MAX_PROCS; i++) {
      if(pcbs[i].flag_autowake) count++;
  }
  return count;
}

// Print out all the queue's info
void ProcessPrintRunQueues(){
  ...
  printf("----------------------------------------------------------------\n");
  printf("Number of %d quantum has passed\n",currentPCB->numquantum);
  for(i = 0; i < NUM_QUEUES; i++) {
    l = AQueueFirst(&runQueue[i]);
    while (l != NULL) {
      pcb = (PCB *)AQueueObject(l);
      printf("Queue %d, Process PID: %ld  has priority %d, runtime is %d, pnice is %d",i , pcb - pcbs, pcb->priority,pcb->runtime, pcb->pnice);
      printf(" and estcpu: %f\n", pcb->estcpu);
      l = AQueueNext(l);
    }
  }
...
void ProcessSchedule () {
  ...
  // if currentPCB in wait queue/ zombieQueue, get new currentPCB from runQueue
  if((currentPCB->flags & PROCESS_STATUS_RUNNABLE))
  {
    // update runtime
    incrRuntime = ClkGetCurJiffies() - currentPCB->switchedtime;
    currentPCB->runtime += incrRuntime;
    
    // print PCB runtime if pinfo is 1
    if(currentPCB->pinfo) printf(PROCESS_CPUSTATS_FORMAT, GetCurrentPid(), currentPCB->runtime, currentPCB->priority);

    // update current PCB estcpu and priority if it does not have to yield
    if(incrRuntime >= PROCESS_QUANTUM_JIFFIES) {
        if(currentPCB->flag_yielding == 0) {
            // if runtime reached time per cpu window
            currentPCB->estcpu++;
            ProcessRecalcPriority(currentPCB);
        }
    }

    currentPCB->numquantum += incrRuntime / PROCESS_QUANTUM_JIFFIES;

    // calculate all priorities if time window exceeded the quanta
    if(( currentPCB->numquantum % CPU_WINDOWS_BETWEEN_DECAYS) == 0) {
      ProcessDecayAllEstcpus();
      ProcessFixRunQueues();
    }
    
    //if the pcb wants to yield,
    if(currentPCB->flag_yielding) {
      // insert currentPCB into new proper queue
      if (ProcessInsertRunning(currentPCB) == QUEUE_FAIL) {
        printf("FATAL ERROR: Cannot insert pcb to queue!\n");
        exitsim();
      }
      // after putting it at the back of the queue, toggle the flag
      currentPCB->flag_yielding = 0;  
      currentPCB = ProcessFindHighestPriorityPCB();
    }

    //if it's idlePCB
    if(currentPCB == idlePCB){
      //move it back to the bottom queue
      ProcessInsertRunning(currentPCB);
  
      currentPCB = ProcessFindHighestPriorityPCB();
      
      if(currentPCB == idlePCB) {
        //if there is no autowake process
        if(ProcessCountAutowake() == 0) {
          // print out processes on wait queue, 
          // so that we know if there is any errors
        ...
        //exit because there is only idlePCB, and no autowake process
        exitsim();
        }
      }
    }
    
    //Wakeup Process that needs to be woken up
    l = AQueueFirst(&waitQueue);
    while (l != NULL) {
      pcb = (PCB *)AQueueObject(l);
      if(pcb->flag_autowake) {
        if(pcb->wakeuptime <= ClkGetCurJiffies())
          ProcessWakeup(pcb);
      }
      l = AQueueNext(l);
    }
    // round-robin
    if(ProcessInsertRunning(currentPCB) == QUEUE_FAIL) {
      printf("FATAL ERROR in ProcessSchedule: Cannot insert pcb to queue!\n");
      exitsim();
    }
  }
  ...
  dbprintf ('p', "Leaving ProcessSchedule (cur=0x%x) switching to %d\n", (int)currentPCB,GetCurrentPid());
}

//----------------------------------------------------------------------
//
...
void ProcessWakeup (PCB *wakeup) {
  ...
  wakeup->flag_autowake = 0;
  if(wakeup -> sleeptime >= 10){
    ProcessDecayEstcpuSleep(wakeup);
    ProcessFixRunQueues();
  }
...
int ProcessFork (VoidFunc func, uint32 param, int pnice, int pinfo,char *name, int isUser) {
  ...
  if (isUser) {
    ...
    // if func is pointing to processIdle(), set base priority to lowest priority
    if(func == ProcessIdle)
    {
...
//
//----------------------------------------------------------------------
int ProcessGetCodeInfo (const char *file, uint32 *startAddr,
