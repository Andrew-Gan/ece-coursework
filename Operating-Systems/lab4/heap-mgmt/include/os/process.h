...
// memory node for buddy memory algo
...
typedef struct PCB {
  ...
  uint32	pagetable[MEM_L1FIELD_PAGE_TABLE_SIZE]; // Statically allocated page table
  int		npages;		// Number of pages allocated to this process
  Link		*l;		// Used for keeping PCB in queues
  short heapTreePtr;
  MemNode heapTree[(MEM_PAGESIZE / MEM_HEAP_BUDDY_SIZE) * 2 - 1];
} PCB;
