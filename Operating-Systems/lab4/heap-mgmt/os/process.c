...
void ProcessFreeResources (PCB *pcb) {
  ...
  // deallocate 4 pages for heap, code and global data
  for( i = 0; i < 5; i++) {
    MemoryFreePte(pcb->pagetable[i]);
  }
  dbprintf('m',"Free code and data at physical page!\n");
  
  // deallocate pages for user stack
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //---------------------------------------------------------
   // allocate 5 pages for heap, user code and global data
  for (i = 0; i < 5; i++) {
  ...
  // setup the binary tree representation of heap memory
  pcb->heapTreePtr = 1;
  pcb->heapTree[0].addr = 4 << MEM_L1FIELD_FIRST_BITNUM;
  pcb->heapTree[0].size = MEM_PAGESIZE;
  pcb->heapTree[0].free_size = MEM_PAGESIZE;
  pcb->heapTree[0].left = -1;
  pcb->heapTree[0].right = -1;
  buildTree(pcb->heapTree, 0, &pcb->heapTreePtr);
