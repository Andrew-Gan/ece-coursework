...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    for(j = 0; j < MEM_L1FIELD_PAGE_TABLE_SIZE; j++) pcbs[i].pagetable[j] = 0;
    pcbs[i].npages = 0;
...
void ProcessFreeResources (PCB *pcb) {
  ...
  // smallest address of allocated pages on user stack
  

  // deallocate 4 pages for code and global data
  for( i = 0; i < 4; i++) {
    MemoryFreePte(pcb->pagetable[i]);
  }
  dbprintf('m',"Free code and data at physical page!\n");
  
  pagenum = pcb->currentSavedFrame[PROCESS_STACK_USER_STACKPOINTER] >> MEM_L1FIELD_FIRST_BITNUM;
  for(i = 4; i < pcb->npages; i++) {
    MemoryFreePte(pcb->pagetable[pagenum]);
    pagenum--;
  }
  dbprintf('m',"Free User stack!\n");
  pcb->npages = 0;

  // deallocate 1 page for system stack
  pagenum = pcb->sysStackArea >> MEM_L1FIELD_FIRST_BITNUM;
  MemoryFreePage(pagenum);
  dbprintf('m',"Free system stack!\n");
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  uint32 physical_page_allocated;
  int user_stack_page;
  ...
  // Set process as parent so that it is forkable
  pcb->isChild = 0;
  ...
  //----------------------------------------------------------------------
  // Allocate 1 page for system stack, 1 page for user stack (at highest of
  ...
  for (i = 0; i < 4; i++) {
    physical_page_allocated = MemoryAllocPage();
    
    if(physical_page_allocated == MEM_FAIL) {
      printf("FATAL ERROR: could allocate page in ProcessFork!\n");
      exitsim();
    }

    dbprintf('m',"Memory allocated for user code and data at page %d\n",physical_page_allocated);

    pcb->pagetable[i] = MemorySetupPte(physical_page_allocated);
  }

  

  // allocate 1 page for user stack
  physical_page_allocated = MemoryAllocPage();
  ...
  dbprintf('m',"Memory allocated for user stack at page %d\n",physical_page_allocated);
  
  user_stack_page = MEM_MAX_VIRTUAL_ADDRESS >> MEM_L1FIELD_FIRST_BITNUM;
  pcb->pagetable[user_stack_page] = MemorySetupPte( physical_page_allocated);

  dbprintf('m',"Memory allocated for user stack at pagetable %d\n",user_stack_page);
  ...
  physical_page_allocated = MemoryAllocPage();
  MemorySetupPte(physical_page_allocated);
  ...
  dbprintf('m',"Memory allocated for system stack at page %d\n",physical_page_allocated);
  
  //set up sysStackArea
  pcb->sysStackArea = physical_page_allocated  << MEM_L1FIELD_FIRST_BITNUM;

  //setup stackframe to be last 4-byte aligned address in physical page of system stack
  stackframe = (uint32 *)((pcb->sysStackArea + MEM_PAGESIZE) & invert((uint32)0x3));
  ...
  // stack frame.
  //----------------------------------------------------------------------
  // setup PTBASE to base address of pagetable
  pcb->currentSavedFrame[PROCESS_STACK_PTBASE] = (uint32)&(pcb->pagetable[0]); // pcb->pagetable[0] & PAGE_PTE_MASK
  // setup PTBITS to level 1 LSB equal to level 2 LSB
  pcb->currentSavedFrame[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM << 16) | MEM_L1FIELD_FIRST_BITNUM;
  // setup PTSIZE to number of entries in pagetable
  pcb->currentSavedFrame[PROCESS_STACK_PTSIZE] = MEM_L1FIELD_PAGE_TABLE_SIZE;
  if (isUser) {
    ...
    //----------------------------------------------------------------------
    pcb->currentSavedFrame[PROCESS_STACK_USER_STACKPOINTER] = MEM_MAX_VIRTUAL_ADDRESS & invert((uint32)0x3);
  ...
  dbprintf ('p', "The Process ID is %d\n", (pcb - pcbs) );
...
void PrintValidPTE(PCB* pcb) {
  int i;
  for(i = 0; i < MEM_L1FIELD_PAGE_TABLE_SIZE; i++) {
    if(pcb->pagetable[i] & MEM_PTE_VALID) {
      printf("pcb->pagetable[%d] = %u\n", i, pcb->pagetable[i]);
    }
  }
}
...
int ProcessRealFork() {
  ...
  dbprintf('m', "(pid: %d) Entering ProcessRealFork()\n", GetCurrentPid());

  if(currentPCB->isChild) return 0;

  dbprintf('m', "Check if any pcb is free in ProcessRealFork\n");

  // exit if no available pcb found
  if (AQueueEmpty(&freepcbs)) {
    printf ("FATAL error: no free processes!\n");
    exitsim ();	// NEVER RETURNS!
  }

  dbprintf('m', "Obtain a free pcb from queue in ProcessRealFork\n");

  // obtain a free pcb
  pcb = (PCB *)AQueueObject(AQueueFirst (&freepcbs));

  // exit if freepcb cannot be removed from queue
  if (AQueueRemove (&(pcb->l)) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not remove link from freepcbsQueue in ProcessRealFork!\n");
    exitsim();
  }

  dbprintf('m', "Incrementing the reference counter in ProcessRealFork\n");

  // increment reference counters and set to read only if PTE is valid
  for(i = 0; i < MEM_L1FIELD_PAGE_TABLE_SIZE; i++) {
    if(currentPCB->pagetable[i] & MEM_PTE_VALID) {
      MemorySetupPte(currentPCB->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
      currentPCB->pagetable[i] |= MEM_PTE_READONLY;
    }
  }

  dbprintf('m', "bcopy PCB in ProcessRealFork\n");

  // copy contents of parent pcb to child pcb
  bcopy((char *)currentPCB, (char *)pcb, sizeof(PCB));

  // allocate 1 new page for system stack
  pcb->sysStackArea = MemoryAllocPage() << MEM_L1FIELD_FIRST_BITNUM;

  dbprintf('m', "Fixing the stack frame in ProcessRealFork\n");

  // fix system stack and current saved frame by preserving offset but changing base address
  pcb->sysStackPtr = (uint32*)(((uint32)currentPCB->sysStackPtr & invert(currentPCB->sysStackArea)) | pcb->sysStackArea);
  pcb->currentSavedFrame = (uint32*)(((uint32)currentPCB->currentSavedFrame & invert(currentPCB->sysStackArea)) | pcb->sysStackArea);

  // bcopy((char*) currentPCB->currentSavedFrame, (char*) pcb->currentSavedFrame, PROCESS_STACK_FRAME_SIZE);
  for(i = 0; i < PROCESS_STACK_FRAME_SIZE; i++) {
    pcb->currentSavedFrame[i] = currentPCB->currentSavedFrame[i];
  }

  // fix PTBASE to base of page table
  pcb->currentSavedFrame[PROCESS_STACK_PTBASE] = (uint32)&(pcb->pagetable[0]);

  // might need to manually fix stackframe elements

  dbprintf('m', "Place child PCB to run queue in ProcessRealFork\n");

  // place child PCB into run queue
  if ((pcb->l = AQueueAllocLink(pcb)) == NULL) {
  ...
  // Q4 requirements: print all valid PTEs in Parent and Child processes
  printf("Printing parent PCB page table (pid: %d)\n", GetCurrentPid());
  PrintValidPTE(currentPCB);
  printf("Printing child PCB page table (pid: %d)\n", pcb - pcbs);
  PrintValidPTE(pcb);

  // set chlid pcb to be runnable
  ProcessSetStatus(pcb, PROCESS_STATUS_RUNNABLE);
