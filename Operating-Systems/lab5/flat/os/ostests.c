...
void RunOSTests() {
  ...
  */
  ...
  if (DfsInodeWriteBytes(handleMeep, writebuff, 357, dstrlen(writebuff)) != dstrlen(writebuff)) {
    printf("ostests: INODE SCENARIO 3 1- failed to write %s to inode %u at position %d\n", writebuff, handleMeep, 357);
    GracefulExit();
  }

  if (DfsInodeReadBytes(handleMeep, readbuff, 357, dstrlen(writebuff)) != dstrlen(writebuff)) {
    printf("ostests: INODE SCENARIO 3 2- failed to read from inode %u at position %d\n", handleMeep, 357);
    GracefulExit();
  }

  if (dstrncmp(readbuff, writebuff, dstrlen(writebuff)) != 0) {
    printf("ostests: INODE SCENARIO 3 3- did not read data correctly from inode %u\n, actual = %s\n", handleMeep, readbuff);
    GracefulExit();
  }
  ...
  if (DfsInodeWriteBytes(handleMeep, writebuff, (DFS_BLOCKSIZE * 3) + 357, dstrlen(writebuff)) != dstrlen(writebuff)) {
    printf("ostests: INODE SCENARIO 3 4- failed to write %s to inode %u at position %d\n", writebuff, handleMeep, (DFS_BLOCKSIZE * 3) + 357);
    GracefulExit();
  }

  if (DfsInodeReadBytes(handleMeep, readbuff, 357, dstrlen(writebuff)) != dstrlen(writebuff)) {
    printf("ostests: INODE SCENARIO 3 5- failed to read from inode %u at position %d\n", handleMeep, (DFS_BLOCKSIZE * 3) + 357);
    GracefulExit();
  }

  if (dstrncmp(readbuff, writebuff, dstrlen(writebuff)) != 0) {
    printf("ostests: INODE SCENARIO 3 6- did not read data correctly from inode %u\n, actual = %s\n", handleMeep, readbuff);
    GracefulExit();
  }

  printf("ostests: INODE SCENARIO 3 7succeeds: readbuff = %s, size of meep.txt = %d\n", readbuff, DfsInodeFilesize(handleMeep));
