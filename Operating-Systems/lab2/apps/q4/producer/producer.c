...
void main (int argc, char *argv[])
{
  ...
  while(strIndex < (sizeof(textStr)-1)){
    ...
    // if buffer is full, wait till space is available
    while(buffer->numElements == sizeof(buffer->buff)) {
    ...
    //Signal the condition variable that we are done
    cond_signal(dataAvailable);
