...
void main (int argc, char *argv[])
{
  ...
  char dataAvailable_str[10];     // Used as command-line argument to pass condition variable handle to finish
  char spaceAvailable_str[10];    // Used as command-line argument to pass condition variable handle to finish  
  ...
  if(dataAvailable = cond_create(l_buffer_proc) == COND_FAIL)
  {
    Printf("Bad cond_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }

  if(spaceAvailable = cond_create(l_buffer_proc) == COND_FAIL)
  {
    Printf("Bad cond_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  ...
  ditoa(dataAvailable, dataAvailable_str);
  ditoa(spaceAvailable, spaceAvailable_str);

  // Now we can create the processes.  Note that you MUST end your call to
  // process_create with a NULL argument so that the operating system
  // knows how many arguments you are sending.
  for(i=0; i<numprocs; i++) {
    process_create("producer.dlx.obj", handle_memory_str, s_proc_completed_str, l_buffer_proc_str, dataAvailable_str, spaceAvailable_str, NULL);
    Printf("Producer's Process %d created\n", i);
    process_create("consumer.dlx.obj", handle_memory_str, s_proc_completed_str, l_buffer_proc_str, dataAvailable_str, spaceAvailable_str, NULL);
