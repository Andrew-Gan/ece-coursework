...
cond_t CondCreate(lock_t lock) {
  ...
  // grabbing a lock should be an atomic operation
  for(con_handle=0; con_handle<MAX_CONDS; con_handle++) {
    if(conds[con_handle].inuse==0) {
      conds[con_handle].inuse = 1;
      break;
    }
  }
  
  if(con_handle==MAX_CONDS) return COND_FAIL;

  //Initialize Conditional Variable
  if (CondInit(&conds[con_handle]) != COND_SUCCESS) return COND_FAIL;

  //assign lock to cond
  conds[con_handle].lock_handle = lock;
...
// helper function to CondCreate
int CondInit(Cond *con) {
  if (!con) return COND_FAIL;
  if (AQueueInit (&con->waiting) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not initialize Cond waiting queue in ConInit!\n");
    exitsim();
  }
  //Might need to initialize more!!
...
//---------------------------------------------------------------------------
int CondHandleWait(cond_t cond_handle) {
  if (cond_handle < 0) return COND_FAIL;
  if (cond_handle >= MAX_CONDS) return COND_FAIL;
  if (!conds[cond_handle].inuse)    return COND_FAIL;
  return CondWait(&conds[cond_handle]);
}
...
int CondWait(Cond* con) {
  ...
  //check if the process has the lock
  if(GetCurrentPid() != locks[con->lock_handle].pid) return COND_FAIL;

  dbprintf ('I', "CondWait: Old interrupt value was 0x%x.\n", intrval);
  dbprintf ('s', "CondWait: Proc %d waiting on cond. var. associated with %d.\n", GetCurrentPid(), (int)(con->lock_handle));
  ...
  // Release the lock
  if (LockHandleRelease(con->lock_handle) == SYNC_FAIL)
  ...
  if(LockHandleAcquire(con->lock_handle) == SYNC_FAIL)
    printf("Error on releasing the lock... \n");
...
//---------------------------------------------------------------------------
int CondHandleSignal(cond_t cond_handle) {
  if (cond_handle < 0) return COND_FAIL;
  if (cond_handle >= MAX_CONDS) return COND_FAIL;
  if (!conds[cond_handle].inuse)return COND_FAIL;
  return CondSignal(&conds[cond_handle]);
}
...
//---------------------------------------------------------------------------
int CondHandleBroadcast(cond_t cond_handle) {
  // Your code goes here
  int intrval;
  ...
  while(!AQueueEmpty(&(conds[cond_handle].waiting))) {
    //call cond signal handle
    CondHandleSignal(cond_handle);
  }
