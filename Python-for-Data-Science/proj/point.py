...
class Point :
    ...
    def distFrom(self, other) :
        ...
        #fill in
        dist = math.sqrt(sum([pow(self.coords[i] - other.coords[i], 2) for i in range(self.dim)]))
        return dist
    ...
    def secondClosest(self, listOfPoints):
        minDist = self.distFrom(listOfPoints[0])
        minPt = listOfPoints[0]
        secondMinPt = listOfPoints[0]
        for p in listOfPoints :
            if (self.distFrom(p) < minDist) :
                secondMinPt = minPt
                minDist = self.distFrom(p)
...
def makePointList(data) :
    #fill in
    pointList = [Point([col for col in row]) for row in data]
    return pointList
