...
void init_I2C1() {
    ...
    I2C1->TIMINGR &= ~(I2C_TIMINGR_PRESC | I2C_TIMINGR_SCLDEL | I2C_TIMINGR_SDADEL);
...
int I2C1_senddata(uint8_t* data, uint32_t size) {
    ...
    if(I2C1->ISR & I2C_ISR_NACKF) {return FAIL;}
    return SUCCESS;
    //---------End-----------
}
...
void write_EEPROM(uint16_t wr_addr, uint8_t data) {
    // Student code goes here
    uint8_t write_buf[3] = {wr_addr>>8, wr_addr & 0xFF, data};
    I2C1_waitidle();
    I2C1_start(0x50, WR);
